...
const getProductDetailsByID = (productDetails, itemId) => productDetails.find(item => item._id === itemId);
        ...
        <Box className={classes.itemRow}>
          <Grid container>
            <Grid item lg={2} md={2} sm={3} xs={12}>
              ...
              <Box>
                <img
                  src={getProductDetailsByID(order.productDetails, item.productId)?.url}
                  className={classes.image}
                  alt={getProductDetailsByID(order.productDetails, item.productId)?.title.shortTitle}
                />
              </Box>
            </Grid>
            <Grid item lg={3} md={3} sm={3} xs={8}>
              {/* Title */}
              <Link to={`/product/${getProductDetailsByID(order.productDetails, item.productId)?._id}`}>
                <Typography className={classes.itemTitle} >
                  {makeShortText(getProductDetailsByID(order.productDetails, item.productId)?.title.longTitle)}
                </Typography>
              </Link>
            </Grid>
